...
const CHANNEL_SOURCE_KINDS = new Set(['broadcastMessage', 'messagePort', 'workerMessage'])
const OBJECT_URL_SOURCE_KINDS = new Set(['objectUrl'])
const PERSISTENT_STORAGE_SOURCE_KINDS = new Set(['indexedDb', 'cacheStorage'])
...
export function selectIastRuleVariant({ runtimePlan = null, sinkId = null, sourceKinds = [], isCrossOrigin = false } = {}) {
  ...
  if ([
    ...
    if (hasAnySourceKind(sourceKinds, CHANNEL_SOURCE_KINDS)) {
      return getRuleVariantByPrefix(sinkPlan, ['channel_'])
    }
  ...
  if (['code.eval', 'code.function.constructor', 'code.function.apply', 'code.setTimeout', 'code.setInterval'].includes(sinkId)) {
    ...
    if (hasAnySourceKind(sourceKinds, PERSISTENT_STORAGE_SOURCE_KINDS)) {
      return getRuleVariantByPrefix(sinkPlan, ['persistent_'])
    }
  ...
  if (['nav.location.href', 'nav.location.assign', 'nav.location.replace', 'nav.window.open'].includes(sinkId)) {
    if (sinkId === 'nav.window.open' && hasAnySourceKind(sourceKinds, OBJECT_URL_SOURCE_KINDS)) {
      return getRuleVariantByPrefix(sinkPlan, ['objecturl_'])
    }
    if (hasAnySourceKind(sourceKinds, CHANNEL_SOURCE_KINDS)) {
      return getRuleVariantByPrefix(sinkPlan, ['channel_'])
    }
  ...
  if (['script.element.src', 'nav.iframe.src', 'worker.webworker.constructor'].includes(sinkId)) {
    if (hasAnySourceKind(sourceKinds, OBJECT_URL_SOURCE_KINDS)) {
      return getRuleVariantByPrefix(sinkPlan, ['objecturl_'])
    }
...
export function buildIastRuntimePlan(modulesJson) {
  ...
  for (const moduleDef of modulesJson.modules) {
    ...
    for (const ruleDef of moduleDef.rules) {
      ...
      if (entry.sources.includes('postMessage') || entry.sources.some((sourceId) => CHANNEL_SOURCE_KINDS.has(sourceId))) {
