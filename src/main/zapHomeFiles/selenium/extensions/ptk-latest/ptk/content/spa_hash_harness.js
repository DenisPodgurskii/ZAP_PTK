...
window.addEventListener('message', (event) => {
    ...
    if (data && data.source === 'ptk-spa-net' && data.event && typeof data.event === 'object') {
        spaNetworkEvents.push({
            kind: data.event.kind || '',
            url: data.event.url || '',
            method: data.event.method || 'GET',
            credentials: data.event.credentials || null,
            requestHeaders: data.event.requestHeaders && typeof data.event.requestHeaders === 'object'
                ? data.event.requestHeaders
                : {},
            bodyPreview: data.event.bodyPreview || null,
            responseUrl: data.event.responseUrl || data.event.url || '',
            responseStatus: data.event.responseStatus ?? null,
            responseHeaders: data.event.responseHeaders && typeof data.event.responseHeaders === 'object'
                ? data.event.responseHeaders
                : {},
            error: data.event.error || null,
            ts: Date.now()
        })
        if (spaNetworkEvents.length > 100) spaNetworkEvents.shift()
    }
})
...
async function runSpaParamTest({ param, payload, checks = [], markerDomain, markerToken }) {
    try {
        ...
        let prototypeBefore = null
        if (checks.includes('prototype_pollution')) {
            const objectProbe = {}
            const arrayProbe = []
            prototypeBefore = {
                objectKeys: new Set(Object.keys(Object.prototype)),
                arrayKeys: new Set(Object.keys(Array.prototype)),
                values: {
                    ptkPolluted: objectProbe.ptkPolluted,
                    polluted: objectProbe.polluted,
                    isAdmin: objectProbe.isAdmin,
                    role: objectProbe.role,
                    method: objectProbe.method,
                    body: objectProbe.body,
                    srcdoc: objectProbe.srcdoc,
                    arrayPtkPolluted: arrayProbe.ptkPolluted
                }
            }
        }
        ...
        const sameOriginRequests = spaNetworkEvents.filter((event) => {
            try {
                const target = new URL(event.responseUrl || event.url, window.location.href)
        ...
        if (checks.includes('spa_csrf')) {
            const csrfCandidates = sameOriginRequests
                .filter((event) => ['POST', 'PUT', 'PATCH', 'DELETE'].includes(String(event.method || 'GET').toUpperCase()))
                .map((event) => {
                    const requestHeaders = event.requestHeaders && typeof event.requestHeaders === 'object'
                        ? event.requestHeaders
                        : {}
                    const lowerHeaderNames = Object.keys(requestHeaders).map((name) => String(name || '').toLowerCase())
                    const hasCsrfHeader = lowerHeaderNames.some((name) => (
                        name.includes('csrf')
                        || name.includes('xsrf')
                        || name === 'x-requested-with'
                        || name.endsWith('-token')
                    ))
                    const bodyPreview = String(event.bodyPreview || '')
                    const hasCsrfBody = /(?:csrf|xsrf|authenticity_token|requesttoken)\b/i.test(bodyPreview)
                    const credentialsMode = String(event.credentials || '').toLowerCase()
                    const sendsCredentials = credentialsMode === 'include' || credentialsMode === 'same-origin'
                    return {
                        url: event.responseUrl || event.url,
                ...
                .filter((entry) => entry.sendsCredentials && !entry.hasCsrfHeader && !entry.hasCsrfBody)
            result.spa_csrf = {
                vulnerable: csrfCandidates.length > 0,
                candidates: csrfCandidates.slice(0, 10)
            }
        }

        if (checks.includes('spa_cors')) {
            const corsCandidates = sameOriginRequests
                .map((event) => {
                    const responseHeaders = event.responseHeaders && typeof event.responseHeaders === 'object'
                        ? event.responseHeaders
                        : {}
                    const allowOrigin = String(responseHeaders['access-control-allow-origin'] || '')
                    const allowCredentials = String(responseHeaders['access-control-allow-credentials'] || '').toLowerCase()
                    return {
                        url: event.responseUrl || event.url,
                        method: String(event.method || 'GET').toUpperCase(),
                        allowOrigin,
                ...
                .filter((entry) => entry.allowOrigin)
            const vulnerableCandidates = corsCandidates.filter((entry) => (
                entry.allowOrigin === '*'
                || (
                    entry.allowOrigin !== window.location.origin
                    && entry.allowOrigin !== 'null'
                )
                || (
            ...
            result.spa_cors = {
                vulnerable: vulnerableCandidates.length > 0,
                candidates: vulnerableCandidates.slice(0, 10),
                observed: corsCandidates.slice(0, 10)
            }
        }
        ...
        if (checks.includes('prototype_pollution')) {
            ...
            const arrayProbe = []
            const entries = []
            const addEntry = (surface, key, value) => {
                if (typeof value === 'undefined') return
                const preview = typeof value === 'string' ? value.slice(0, 120) : JSON.stringify(value)
                entries.push({ surface, key, preview })
            }
            const watchKeys = ['ptkPolluted', 'polluted', 'isAdmin', 'role', 'method', 'body', 'srcdoc']
            for (const key of watchKeys) {
                const beforeValue = prototypeBefore?.values?.[key]
                const currentValue = objectProbe[key]
                if (typeof currentValue !== 'undefined' && currentValue !== beforeValue) {
                    if (!markerToken || String(currentValue).includes(markerToken) || currentValue === true) {
                        addEntry('object', key, currentValue)
                    }
                }
            }
            if (typeof arrayProbe.ptkPolluted !== 'undefined' && arrayProbe.ptkPolluted !== prototypeBefore?.values?.arrayPtkPolluted) {
                if (!markerToken || String(arrayProbe.ptkPolluted).includes(markerToken) || arrayProbe.ptkPolluted === true) {
                    addEntry('array', 'ptkPolluted', arrayProbe.ptkPolluted)
                }
            }
            const newObjectKeys = Object.keys(Object.prototype).filter((key) => !prototypeBefore?.objectKeys?.has(key))
            const newArrayKeys = Object.keys(Array.prototype).filter((key) => !prototypeBefore?.arrayKeys?.has(key))
            newObjectKeys.slice(0, 5).forEach((key) => addEntry('object', key, Object.prototype[key]))
            newArrayKeys.slice(0, 5).forEach((key) => addEntry('array', key, Array.prototype[key]))
            result.prototype_pollution = {
                vulnerable: entries.length > 0,
