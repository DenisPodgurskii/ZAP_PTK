...
export class ptk_exporter {
  ...
  <MacroEvent>
    ...
    getRequestHeaderValue(requestHeaders = [], name = '') {
        if (!Array.isArray(requestHeaders) || !name) return ''
        const target = String(name || '').trim().toLowerCase()
        const header = requestHeaders.find((item) => String(item?.name || '').trim().toLowerCase() === target)
        return String(header?.value || '')
    }

    isMultipartRequest(requestHeaders = [], requestBody = null) {
        const requestContentType = this.getRequestHeaderValue(requestHeaders, 'content-type')
    ...
    decodeRawRequestBody(rawParts = null) {
    ...
    serializeFormData(formData = null) {
    ...
    getEntryPostData(item) {
        ...
        if (item.request.requestBody) {
            ...
            const isMultipart = this.isMultipartRequest(item.requestHeaders, item.request.requestBody)
            const rawBody = typeof item.request.requestBody.raw === 'string'
                ? item.request.requestBody.raw
                : this.decodeRawRequestBody(item.request.requestBody.raw)
            if (rawBody) {
                postData.text = rawBody
                bodySize = postData.text.length
            } else if (item.request?.requestBody.postData) {
                postData.text = item.request.requestBody.postData
                bodySize = postData.text.length
            } else if (item.request.requestBody.formData && !isMultipart) {
                let formData = item.request.requestBody.formData
                postData.text = this.serializeFormData(formData)
