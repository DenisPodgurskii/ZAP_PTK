...
function decodeRawRequestBody(rawParts = null) {
...
function extractStructuredRequestBody(requestObj = {}) {
    ...
    if (Array.isArray(requestBody?.raw) && requestBody.raw.length) {
        return decodeRawRequestBody(requestBody.raw)
    }
    const normalizedHeaders = normalizeStructuredHeaders(requestObj?.headers || requestObj?.requestHeaders || {})
    const bodyContentType = String(requestBody?.contentType || requestBody?.mimeType || "").toLowerCase()
    const requestContentType = String(normalizedHeaders["content-type"] || "").toLowerCase()
    const isMultipart = bodyContentType.includes("multipart/form-data") || requestContentType.includes("multipart/form-data")
