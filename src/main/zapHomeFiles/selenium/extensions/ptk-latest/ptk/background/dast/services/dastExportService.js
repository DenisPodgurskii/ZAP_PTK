...
export class DastExportService {
    ...
    async createChunkedExport(scanResult, { target = "download", fileName = "PTK_DAST_scan.json" } = {}) {
        if (!scanResult || typeof scanResult !== "object") return null
        const payload = this.lifecycleService?.buildExportPayload?.(scanResult, { target })
