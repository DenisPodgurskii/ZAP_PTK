...
export class DastResultProjector {
            ...
            summary.evidence = {
                dast: this.sanitizeUiValue({
                    ...
                    occurrenceCount: Number.isFinite(Number(dastEvidence.occurrenceCount))
                        ...
                        : null,
                    sampleLimit: Number.isFinite(Number(dastEvidence.sampleLimit))
                        ? Number(dastEvidence.sampleLimit)
                        : null,
                    truncated: dastEvidence.truncated === true,
                    samples: Array.isArray(dastEvidence.samples) ? dastEvidence.samples : null
                }, 0, this.limits)
            }
        }
        return summary
    ...
    buildScanResultUiSnapshot(scanResult) {
        ...
        if (Array.isArray(source.moduleStatuses) && source.moduleStatuses.length) {
            snapshot.moduleStatuses = source.moduleStatuses
                .slice(0, this.limits.maxArrayItems)
                .map((status) => this.sanitizeUiValue(status, 0, this.limits))
                .filter(Boolean)
        }
        if (source.settings) {
