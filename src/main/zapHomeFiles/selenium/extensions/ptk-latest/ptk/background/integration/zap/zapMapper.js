...
function mapSeverityForZap(severity) {
...
export function toAlert(finding, { engine, scanId } = {}) {
    ...
    const ptkSeverity = normalizeSeverity(finding.severity || finding.effectiveSeverity)
    const severity = mapSeverityForZap(ptkSeverity)
    const riskId = clamp(mapRiskIdFromSeverity(severity), 1, 3)
    ...
    const tags = buildTags(finding, normalizedEngine, ptkSeverity)
