...
class ScanResultStore {
    ...
    findLatestScanId({ engine = null, host = null, tabId = null } = {}) {
        const ids = this.findScanIds({ engine, host, tabId, latestFirst: true })
        return ids[0] || null
    }

    findScanIds({ engine = null, host = null, tabId = null, startedAfterMs = null, latestFirst = false } = {}) {
        const engineKey = normalizeEngineName(engine)
        const hostKey = normalizeHostKey(host)
        const hasTabHint = tabId !== undefined && tabId !== null
        const minStartedAt = Number(startedAfterMs)
        const enforceStartedAfter = Number.isFinite(minStartedAt) && minStartedAt > 0
        const matches = []
        ...
        this._scans.forEach((scan, scanId) => {
            ...
            const startedTs = toTimestamp(scan.startedAt || null)
            if (enforceStartedAfter && startedTs > 0 && startedTs < minStartedAt) return

            const tabBoost = hasTabHint && scan.tabId === tabId ? 1 : 0
            const ts = toTimestamp(scan.finishedAt || scan.finished || scan.startedAt || null)
            matches.push({
                scanId: String(scanId),
                score: (tabBoost * 10_000_000_000_000) + ts
            })
        })

        matches.sort((a, b) => latestFirst ? b.score - a.score : a.score - b.score)
        return matches.map(entry => entry.scanId)
    }
