...
import { PRO_UI_VISIBLE, PORTAL_ACTIONS_VISIBLE } from "./releaseFeatureFlags.js"
...
function getIastRuntimeHealth(source = null) {
    const scanResult = source?.scanResult || source || controller?.scanViewModel || controller?.scanResult?.scanResult || {}
    const activation = scanResult?.iastTelemetry?.activation && typeof scanResult.iastTelemetry.activation === "object"
        ? scanResult.iastTelemetry.activation
        : {}
    const runtimeHealth = activation?.runtimeHealth && typeof activation.runtimeHealth === "object"
        ? activation.runtimeHealth
        : null
    return runtimeHealth
}

function formatIastRuntimeHealthSummary(runtimeHealth = null, { compact = false, includeNormal = false } = {}) {
    const state = String(runtimeHealth?.state || "").trim().toLowerCase()
    const degradedCount = Array.isArray(runtimeHealth?.degradedHookGroups) ? runtimeHealth.degradedHookGroups.length : 0
    const disabledCount = Array.isArray(runtimeHealth?.globallyDisabledHookGroups) ? runtimeHealth.globallyDisabledHookGroups.length : 0
    const error = String(runtimeHealth?.error || "").trim()
    if (!state || state === "normal") {
        return includeNormal ? "IAST runtime healthy" : ""
    }
    if (state === "initializing") {
        return compact ? "IAST runtime initializing" : "IAST runtime is still initializing."
    }
    if (state === "degraded") {
        const countLabel = degradedCount > 0 ? `${degradedCount} hook group${degradedCount === 1 ? "" : "s"} limited` : "some hook groups limited"
        return compact
            ? `IAST runtime degraded for stability (${countLabel})`
            : `IAST runtime degraded for stability. ${countLabel.charAt(0).toUpperCase()}${countLabel.slice(1)}.`
    }
    if (state === "disabled_due_to_runtime_errors") {
        const countLabel = disabledCount > 0 ? `${disabledCount} high-risk hook group${disabledCount === 1 ? "" : "s"} disabled` : "some high-risk hook groups disabled"
        return compact
            ? `IAST runtime limited for stability (${countLabel})`
            : `IAST runtime limited for stability. ${countLabel.charAt(0).toUpperCase()}${countLabel.slice(1)}.`
    }
    if (state === "agent_failed") {
        return error
            ? `IAST runtime agent failed to attach: ${error}`
            : "IAST runtime agent failed to attach."
    }
    return ""
}

function resetIastRuntimeHealthUi() {
    $("#scanning_label").text("Scanning")
    $("#iast_runtime_health_status")
        .hide()
        .text("")
        .css("color", "")
}

function updateIastRuntimeHealthUi(result, viewState = resolveIastViewState(result)) {
    const runtimeHealth = getIastRuntimeHealth(result)
    const summary = formatIastRuntimeHealthSummary(runtimeHealth, { compact: false })
    const state = String(runtimeHealth?.state || "").trim().toLowerCase()
    const isRunning = viewState === "running"
    const hasRenderableData = viewState === "idle_with_data"
    const shouldShowBanner = isRunning || (!!summary && hasRenderableData)
    $(".scan_info").toggle(shouldShowBanner)
    $("#scanning_label").text(isRunning ? "Scanning" : "Last runtime target")
    $("#scanning_url").text(result?.scanResult?.host || controller?.scanViewModel?.host || controller?.scanResult?.scanResult?.host || "")
    const $status = $("#iast_runtime_health_status")
    if (!summary) {
        $status.hide().text("").css("color", "")
        return
    }
    const color = state === "disabled_due_to_runtime_errors" || state === "agent_failed"
        ? "#9f3a38"
        : state === "degraded"
            ? "#b58105"
            : "rgba(0,0,0,.6)"
    $status.text(summary).css("color", color).show()
}
...
function composeIastProgressStatus(info = {}) {
    ...
    const runtimeHealth = info.runtimeHealth || getIastRuntimeHealth(info)
    const runtimeHealthSummary = formatIastRuntimeHealthSummary(runtimeHealth, { compact: true })
    if (message && host) {
        return runtimeHealthSummary ? `${message}: ${host} | ${runtimeHealthSummary}` : `${message}: ${host}`
    }
    if (message) return runtimeHealthSummary ? `${message} | ${runtimeHealthSummary}` : message
    if (host) return runtimeHealthSummary ? `Runtime scan running on ${host} | ${runtimeHealthSummary}` : `Runtime scan running on ${host}`
    if (runtimeHealthSummary) return runtimeHealthSummary
...
const $portalScansModal = $('#portal_scans_modal')
...
let portalUploadAvailable = false

function resetPortalScansModalState({ clearTable = false, clearErrors = false } = {}) {
    $portalScansModal.removeClass('loading')
    $portalScansModal.find('.ui.dimmer').removeClass('active visible transition fade in').hide()
    if (clearErrors) {
...
function setPortalScansTab(tabName) {
    const normalized = tabName === 'download' ? 'download' : 'upload'
    $('#portal_scans_modal .ptk-portal-tabs .item').removeClass('active')
    $(`#portal_scans_modal .ptk-portal-tabs .item[data-portal-tab="${normalized}"]`).addClass('active')
    $('#portal_scans_modal .ptk-portal-tab-panel').removeClass('active')
    $(`#portal_scans_modal .ptk-portal-tab-panel[data-portal-tab-panel="${normalized}"]`).addClass('active')
}

function setPortalUploadAvailability(canUpload) {
    portalUploadAvailable = !!canUpload
    $('#portal_upload_unavailable').toggle(!portalUploadAvailable)
    $('#portal_upload_scan_button').toggleClass('disabled', !portalUploadAvailable)
}
...
function rebuildIastProjectDropdown(projectOptions) {
    ...
    if ($iastSaveScanProjectDropdown && $iastSaveScanProjectDropdown.length) {
        $iastSaveScanProjectDropdown.off('change').on('change', function () {
            hideIastSaveScanModalError()
            setDownloadScansError('')
            clearDownloadScansTable()
        })
...
function runIastSaveScan(projectId, $loader) {
    ...
    controller.saveScan(projectId).then((result) => {
        ...
        resetPortalScansModalState()
    })
}

function handlePortalUploadRequest($loader) {
    hideIastSaveScanModalError()
    if (!portalUploadAvailable) {
        showIastSaveScanModalError('No scan results available to upload.')
        return
    }
...
function loadIastPortalProjects() {
    hideIastSaveScanModalError()
    setDownloadScansError('')
    clearDownloadScansTable()
    $portalScansModal.addClass('loading')
    fetchIastPortalProjects()
        .then((options) => {
            rebuildIastProjectDropdown(options)
        })
        .catch((err) => {
            const message = err?.message || 'Unable to load projects. Check your PTK Pro configuration.'
            showIastSaveScanModalError(message)
        })
        .finally(() => {
            resetPortalScansModalState()
        })
...
jQuery(function () {
    ...
    $portalScansModal.modal({
        allowMultiple: true,
        onHidden: function () {
            resetPortalScansModalState({ clearErrors: true })
        },
        onShow: function () {
            resetPortalScansModalState({ clearTable: true, clearErrors: true })
        }
    })

    $(document).on('click', '#portal_upload_scan_button', function () {
        handlePortalUploadRequest()
        return false
    })

    $(document).on('click', '#portal_download_scans_button', function () {
        hideIastSaveScanModalError()
        const projectId = $iastSaveScanProjectDropdown.val()
        if (!projectId) {
            setDownloadScansError('Select a project to load scans.')
        ...
        const payloadProjectId = iastSaveScanProjectMap.get(projectId) ?? projectId
        loadIastScansForProject(payloadProjectId)
        return false
    })

    $(document).on("click", ".portal_scans_button", function () {
        if (!PORTAL_ACTIONS_VISIBLE) return false
        resetPortalScansModalState({ clearTable: true, clearErrors: true })
        $portalScansModal.modal('show')
        loadIastPortalProjects()
        return false
    })
...
function showRunningForm(result) {
    ...
    updateIastRuntimeHealthUi(result, 'running')
}
...
function showScanForm(result) {
    ...
    updateIastRuntimeHealthUi(result, 'idle_with_data')
}
...
function changeView(result) {
    ...
    if (viewState === 'running') {
        ...
        scheduleIastProgressUpdate({ message: "Runtime scan running", scanResult: result.scanResult || {}, runtimeHealth: getIastRuntimeHealth(result) })
    }
    ...
    updateIastRuntimeHealthUi(result, viewState)
}
...
function bindScanResult(result) {
    ...
    updateIastRulepackUi(result)
    updateIastRuntimeHealthUi(result, resolveIastViewState(result))
    if (!result.isScanRunning) {
        resetIastProgressRender({ hide: true })
    }
    $('#request_info').html("")
