...
class rbuilderUI {
    ...
    buildFromDetails(request) {
        if (request && request.requestHeaders) {
            ...
            let $form = $('#request_form')
            let headersStr = typeof request?.raw === 'string' && request.raw.trim().length
                ? request.raw
                : ''
            if (!headersStr) {
                ...
                if (request?.requestBody?.raw) {
                    ...
                    const contentTypeHeader = Array.isArray(request.requestHeaders)
                        ? request.requestHeaders.find((header) => String(header?.name || '').toLowerCase() === 'content-type')
                        : null
                    const contentType = String(contentTypeHeader?.value || '')
...
browser.runtime.onMessage.addListener(function (message, sender, sendResponse) {
    ...
    if (message.channel == "ptk_background2popup_rbuilder") {
        ...
        if (message.type == "scan_failed") {
            $('#scan_error_message').text(message.error || 'Could not scan the current request.')
            $('.mini.modal').modal('show')
        }
