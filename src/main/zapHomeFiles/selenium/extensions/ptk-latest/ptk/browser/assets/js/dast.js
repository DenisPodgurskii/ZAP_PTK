...
const DAST_RENDER = {
    ...
    renderedAggregatedFindingIds: new Set(),
...
function applyDastPortalPolicyState(result = {}) {
    ...
    const normalizedPreviousValue = String(previousSelectedValue || '').trim().toUpperCase()
    const previousBuiltinValue = DAST_BUILTIN_POLICY_OPTIONS.some((entry) => entry.value === normalizedPreviousValue)
    ...
    const selectedValue = policyState?.selectedPolicy?.id
        ...
        : (previousBuiltinValue || getCurrentDastBuiltinPolicyValue(result, previousSelectedValue))
...
function resolvePassiveAggregateScopeHost(attack = null, original = null) {
    const candidates = [
        attack?.request?.url,
        original?.request?.url,
        attack?.finding?.location?.runtimeUrl,
        attack?.finding?.location?.url
    ]
    for (const candidate of candidates) {
        ...
        try {
            ...
            const hostMatch = raw.match(/^[a-z]+:\/\/([^/]+)/i)
            if (hostMatch?.[1]) {
                return String(hostMatch[1]).trim().toLowerCase()
            }
        }
    }
    return null
...
function getPassiveHeadersAggregateFindingKey(attack = null, original = null) {
    const moduleId = String(attack?.moduleId || attack?.finding?.moduleId || attack?.metadata?.moduleId || '').trim().toLowerCase()
    ...
    const ruleId = String(attack?.ruleId || attack?.finding?.ruleId || attack?.metadata?.ruleId || attack?.metadata?.id || '').trim().toLowerCase()
    const aggregateScopeHost = String(attack?.finding?.evidence?.dast?.aggregate?.scopeHost || '').trim().toLowerCase()
    const scopeHost = aggregateScopeHost || resolvePassiveAggregateScopeHost(attack, original)
    if (!ruleId || !scopeHost) return null
    return ['scan', moduleId, ruleId, scopeHost].join('|')
}
...
function shouldRenderAttackCard(attack = null, original = null) {
    ...
    const aggregatedFindingKey = getPassiveHeadersAggregateFindingKey(attack, original)
    if (!aggregatedFindingKey) {
        return true
    }
    if (DAST_RENDER.renderedAggregatedFindingIds.has(aggregatedFindingKey)) {
        return false
    }
    DAST_RENDER.renderedAggregatedFindingIds.add(aggregatedFindingKey)
    return true
}
...
function renderGroupedReconRoutes(entry) {
    ...
    const renderRouteItem = (route) => {
        const label = `${route.method} ${route.path}`
        if (route.url) {
            return `<li><a class="ptk-card-link" href="${ptk_utils.escapeHtml(route.url)}" target="_blank" rel="noreferrer">${ptk_utils.escapeHtml(label)}</a></li>`
        }
...
function buildGroupedReconCardHtml(entry) {
    ...
    const html = `
        <div class="ui message attack_info grouped_recon_card ${meta.attackClass} ${ptk_utils.escapeHtml(safeRequestId)}"
            ...
            ${targetUrl ? `<div class="description"><p>Sample URL: <a class="ptk-card-link" href="${safeTarget}" target="_blank" rel="noreferrer">${safeTarget}</a></p></div>` : ''}
            ${routesHtml}
...
jQuery(function () {
    ...
    function resetPortalScansModalState({ clearTable = false, clearErrors = false } = {}) {
        ...
        if (clearErrors) {
            ...
            setDownloadScansError('', { showPortalHint: false })
        }
        if (clearTable) {
    ...
    function populateSaveScanProjectDropdown(projectOptions) {
        ...
        if ($saveScanProjectDropdown && $saveScanProjectDropdown.length) {
            $saveScanProjectDropdown.off('change').on('change', function () {
                hideSaveScanModalError()
    ...
    function setDownloadScansError(message, options = {}) {
        if (message) {
            showSaveScanModalError(message)
        } else {
            hideSaveScanModalError()
        }
    }
    ...
    function loadPortalProjects() {
        ...
        fetchPortalProjects()
            .then(options => {
                populateSaveScanProjectDropdown(options)
            })
            .catch(err => {
                const message = err?.message || 'Unable to load projects. Check your PTK+ configuration.'
                showSaveScanModalError(message)
                setDownloadScansError(message, { showPortalHint: /api key|token|configuration/i.test(message) })
            })
            .finally(() => {
    ...
    function loadScansForProject(projectId) {
        if (!projectId) {
            setDownloadScansError('Select a project to load scans.', { showPortalHint: false })
            clearDownloadScansTable()
            return
        }
        setDownloadScansError('', { showPortalHint: false })
        ...
        controller.downloadScans(projectId, 'dast').then(result => {
            if (!result?.success) {
                ...
                setDownloadScansError(message, { showPortalHint: result?.error === 'missing_api_key' })
                clearDownloadScansTable()
                return
            }
            setDownloadScansError('', { showPortalHint: false })
            ...
            setDownloadScansError(err?.message || 'Unable to load scans.', { showPortalHint: /api key|token|configuration/i.test(err?.message || '') })
            clearDownloadScansTable()
        }).finally(() => {
            resetPortalScansModalState()
        })
    ...
    function handlePortalUploadRequest($loader) {
        ...
        if (!portalUploadAvailable) {
            showSaveScanModalError('No scan results available to upload.')
            return
        }
    ...
    function requestProjectsAndShowModal() {
        openPortalScansModal()
    }
    ...
    $(document).on('click', '#portal_download_scans_button', function () {
        ...
        const payloadProjectId = saveScanProjectMap.get(projectId) ?? projectId
        loadScansForProject(payloadProjectId)
        return false
    })
    ...
    $(document).on("click", ".attack_details", async function () {
        ...
        if (!hasFindingPresentationData(enrichedAttack) || (
            String(enrichedAttack?.presentationAggregate || enrichedAttack?.finding?.presentationAggregate || '').trim().toLowerCase() === 'scan'
            && !hasAggregatedFindingSamples(enrichedAttack)
        )) {
...
function hasAggregatedFindingSamples(attack) {
    const dastEvidence = attack?.finding?.evidence?.dast
    return !!(dastEvidence && typeof dastEvidence === 'object' && Array.isArray(dastEvidence.samples) && dastEvidence.samples.length)
}
...
async function ensureAttackFindingDetails(attack, requestId, attackId) {
    ...
    const isScanAggregate = String(
        attack?.presentationAggregate
        || attack?.finding?.presentationAggregate
        || attack?.metadata?.presentationAggregate
        || attack?.metadata?.extensions?.ptk?.presentation?.aggregate
        || ''
    ).trim().toLowerCase() === 'scan'
    if (hasFindingPresentationData(attack) && (!isScanAggregate || hasAggregatedFindingSamples(attack))) {
        return lookup
    }
    try {
        const details = await controller.getFindingDetails({
...
function seedRenderedFromViewModel(viewModel) {
    ...
    DAST_RENDER.renderedAggregatedFindingIds.clear()
