...
jQuery(function () {
  ...
  function rebuildSastProjectDropdown(projectOptions) {
    ...
    if ($sastSaveScanProjectDropdown && $sastSaveScanProjectDropdown.length) {
      $sastSaveScanProjectDropdown.off('change').on('change', function () {
        hideSastSaveScanModalError()
        setDownloadScansError('')
        clearDownloadScansTable()
      })
  ...
  function handlePortalUploadRequest($loader) {
    ...
    if (!portalUploadAvailable) {
      showSastSaveScanModalError('No scan results available to upload.')
      return
    }
  ...
  function loadSastPortalProjects() {
    hideSastSaveScanModalError()
    setDownloadScansError('')
    clearDownloadScansTable()
    $portalScansModal.addClass('loading')
    fetchSastPortalProjects()
      ...
      .catch(err => {
        ...
        showSastSaveScanModalError(message)
      })
      .finally(() => {
        resetPortalScansModalState()
      })
  ...
  $(document).on('click', '#portal_download_scans_button', function () {
    ...
    const payloadProjectId = sastSaveScanProjectMap.get(projectId) ?? projectId
    loadSastScansForProject(payloadProjectId)
    return false
  });
