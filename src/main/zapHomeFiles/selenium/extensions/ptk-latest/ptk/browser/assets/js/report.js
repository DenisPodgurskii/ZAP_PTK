...
function buildSummaryTableRows(model, { interactive = true } = {}) {
    ...
    const rows = engines.map(engine => {
        ...
        const renderSeverityCell = (severity) => {
            ...
            if (!interactive || !hasFilter) return String(count)
        ...
        return {
            isTotal: false,
            cells: [
                interactive ? escapeText(String(engine || "UNKNOWN"), "") : String(engine || "UNKNOWN"),
...
function getFilteredExportModel() {
    const filtered = {
        ...
        discoveries: {
            sast: Array.isArray(exportModel.discoveries?.sast) ? exportModel.discoveries.sast.slice() : []
        },
        summary: {
            byEngine: {},
            bySeverity: {}
...
function buildReportModel(sourceModel, options = {}) {
    ...
    const model = {
        ...
        discoveries: {
            sast: Array.isArray(sourceModel.discoveries?.sast) ? sourceModel.discoveries.sast : []
        },
        findings: [],
...
function renderExecutiveView() {
    ...
    const tiers = { confirmed: 0, likely: 0, potential: model.findings.length, confirmedHigh: 0 }
...
function replaceSectionDiscoveries(sectionId, discoveries) {
    if (!exportModel.discoveries || typeof exportModel.discoveries !== "object") {
        exportModel.discoveries = { sast: [] }
    }
    exportModel.discoveries[sectionId] = Array.isArray(discoveries) ? discoveries : []
    updateExportButtons()
}
...
function buildExportFindingFromNormalized(finding, viewModel) {
    ...
    if (engine === "DAST") {
        ...
        const requestRaw = attackRecord?.request?.raw
            || requestRecord?.original?.request?.raw
            || (attackRecord?.request ? buildRawRequest(attackRecord.request) : "")
            || (requestRecord?.original?.request ? buildRawRequest(requestRecord.original.request) : "")
...
function buildMarkdownFromExportModel(model) {
    ...
    if (isExecutive) {
        ...
        const discoveryItems = Array.isArray(model.discoveries?.sast) ? model.discoveries.sast : []
        if (discoveryItems.length) {
            push(`## SAST Discovery`)
            push(``)
            discoveryItems.forEach(item => {
                push(`### ${escapeMarkdownText(item.title || "Discovery")}`)
                if (item.file || item.location) {
                    const locationLabel = [
                        item.file ? `File: ${escapeMarkdownText(item.file)}` : "",
                        item.location ? `Location: ${escapeMarkdownText(formatRange(item.location))}` : ""
                    ].filter(Boolean).join(" | ")
                    if (locationLabel) {
                        push(locationLabel)
                        push(``)
                ...
                if (item.summary) {
                    push(escapeMarkdownText(item.summary))
                    push(``)
                }
                ...
                if (detailItems.length) {
                    ...
                    detailItems.forEach(detail => {
                        push(`- ${escapeMarkdownText(detail)}`)
                    })
                    push(``)
                }
                push(`---`)
...
function buildPdfFromExportModel(model) {
    ...
    if (Object.keys(model.summary.byEngine || {}).length) {
        ...
        const rows = buildSummaryTableRows(model, { interactive: false })
        if (rows.length) {
            addAutoTable({
    ...
    const discoveryItems = Array.isArray(model.discoveries?.sast) ? model.discoveries.sast : []
    if (discoveryItems.length) {
        addSectionTitle("SAST Discovery")
        discoveryItems.forEach(item => {
            ensureSpace(100)
            setBody(doc, pdfTheme)
            doc.setFont("helvetica", "bold")
            doc.setFontSize(10)
            const titleText = clampCellText(item.title || "Discovery", { maxChars: 100, maxLines: 1 })
            doc.text(titleText, margin, cursorY)
            cursorY += 14

            const metaLine = [
                item.file ? `File: ${item.file}` : "",
                item.location ? `Location: ${formatRange(item.location)}` : ""
            ].filter(Boolean).join(" | ")
            if (metaLine) {
                doc.setFont("helvetica", "normal")
                doc.setFontSize(9)
                doc.setTextColor(120, 120, 120)
                const metaLines = doc.splitTextToSize(metaLine, layout.pageWidth - margin * 2)
                doc.text(metaLines.slice(0, 2), margin, cursorY)
                cursorY += metaLines.slice(0, 2).length * 11
                doc.setTextColor(0)
            }
            ...
            if (item.summary) {
                ...
                doc.setFont("helvetica", "normal")
                doc.setFontSize(9)
                const summaryLines = doc.splitTextToSize(item.summary, layout.pageWidth - margin * 2)
                doc.text(summaryLines, margin, cursorY)
                cursorY += summaryLines.length * 10 + 6
            }
...
function buildExecutivePdfFromReportModel(model) {
    ...
    if (Object.keys(model.summary.byEngine || {}).length) {
        ...
        cursorY = riskEndY + pdfTheme.spacing.sm
        const summaryRows = buildSummaryTableRows(model, { interactive: false })
        if (summaryRows.length) {
            addAutoTable({
                head: [["Engine", "Critical", "High", "Medium", "Low", "Info", "Total"]],
                body: summaryRows.map(row => row.cells),
                columnStyles: { 0: { cellWidth: 70 } }
            })
        }
...
function discoveryText(value, fallback = "") {
    ...
    if (Array.isArray(value)) {
        return value.map((entry) => discoveryText(entry, "")).filter(Boolean).join(", ")
    }
    if (value && typeof value === "object") {
        if (value.__type === "truncated_string" && typeof value.preview === "string") {
...
function discoveryList(value, max = 6) {
    const raw = Array.isArray(value) ? value : (value === undefined || value === null ? [] : [value])
    const seen = new Set()
    const list = []
    raw.forEach((entry) => {
        const text = discoveryText(entry, "").trim()
        if (!text || seen.has(text)) return
        seen.add(text)
        list.push(text)
    })
    if (max > 0 && list.length > max) {
        return list.slice(0, max).concat(`+${list.length - max} more`)
    }
    return list
}

function discoveryLocation(entry) {
    return entry?.location || entry?.loc || entry?.sourceLoc || null
}

function discoveryFile(entry) {
    return discoveryText(entry?.file || entry?.sourceFile || "", "")
}

function detailLine(label, value) {
    const text = Array.isArray(value)
        ? discoveryList(value).join(", ")
        : discoveryText(value, "")
    return text ? `${label}: ${text}` : ""
}

function flattenSastDiscovery(codeArtifacts) {
    const sast = codeArtifacts?.sast
    if (!sast || typeof sast !== "object") return []
    const entries = []
    const pushEntry = (category, entry, index, title, summary, details = []) => {
        ...
        entries.push({
            ...
            file: discoveryFile(entry),
            location: discoveryLocation(entry)
        })
    }

    ;(sast.routes || []).forEach((entry, index) => {
        const title = `Route ${discoveryText(entry.path, "unknown")}`
        const summaryParts = []
        if (entry.adminLike) summaryParts.push("admin-like")
        if ((entry.authHints || []).length) summaryParts.push("auth-gated")
        const summary = summaryParts.length
            ? `Client route with ${summaryParts.join(", ")} hints.`
            : "Client route discovered from router configuration."
        pushEntry("route", entry, index, title, summary, [
            detailLine("Route key", entry.routeKey),
            detailLine("Auth hints", entry.authHints),
            detailLine("Protocols", entry.protocolHints),
            detailLine("Environment", entry.environmentHints)
        ])
    })

    ;(sast.endpoints || []).forEach((entry, index) => {
        const method = discoveryText(entry.method, "GET")
        const url = discoveryText(entry.resolvedUrl || entry.url, "unknown")
        const title = `${method} ${url}`
        const summaryParts = [discoveryText(entry.transport, "request")]
        if ((entry.paramNames || []).length) summaryParts.push(`params ${discoveryList(entry.paramNames).join(", ")}`)
        if ((entry.bodyKeys || []).length) summaryParts.push(`body ${discoveryList(entry.bodyKeys).join(", ")}`)
        const summary = `${summaryParts.join(" | ")}`
        pushEntry("endpoint", entry, index, title, summary, [
            detailLine("Headers", entry.headerNames),
            detailLine("Auth hints", entry.authHints),
            detailLine("Protocols", entry.protocolHints),
            detailLine("Discovery", entry.discoveryTags),
            detailLine("Environment", entry.environmentHints),
            detailLine("Storage", entry.storageHints),
            detailLine("Uploads", entry.uploadSignals),
            entry.adminLike ? "Admin-like endpoint hint" : "",
            detailLine("Route key", entry.routeKey)
        ])
    })

    ;(sast.graphql || []).forEach((entry, index) => {
        const opName = discoveryList(entry.operationNames, 3).join(", ") || "anonymous"
        const opTypes = discoveryList(entry.operationTypes, 3).join(", ") || "query"
        const title = `GraphQL ${opTypes}: ${opName}`
        const summary = discoveryText(entry.resolvedUrl || entry.url || entry.routeKey, "GraphQL client surface")
        pushEntry("graphql", entry, index, title, summary, [
            detailLine("Transport", entry.transport),
            detailLine("Variables", entry.variableNames),
            detailLine("Root fields", entry.rootFields),
            detailLine("Auth hints", entry.authHints),
            entry.adminLike ? "Admin-like GraphQL hint" : ""
        ])
    })

    ;(sast.surfaces || []).forEach((entry, index) => {
        const title = `Surface ${discoveryText(entry.surfaceType, "hint")}: ${discoveryText(entry.label, "unknown")}`
        const summary = "Potentially gated client-side feature or debug/admin surface."
        pushEntry("surface", entry, index, title, summary, [
            detailLine("Hints", entry.hintNames),
            detailLine("Protocols", entry.protocolHints),
            detailLine("Occurrences", entry.occurrenceCount),
            detailLine("Files", entry.sourceFiles),
            entry.adminLike ? "Admin-like surface" : "",
            detailLine("Route key", entry.routeKey)
        ])
    })

    ;(sast.hiddenParams || []).forEach((entry, index) => {
        const title = `Hidden parameter ${discoveryText(entry.paramName, "unknown")}`
        const actionText = discoveryList(entry.actions || entry.action).join(", ") || discoveryText(entry.action, "access")
        const summary = `Observed via ${discoveryText(entry.container, "unknown")} ${actionText}.`
        pushEntry("hidden_param", entry, index, title, summary, [
            detailLine("Hint types", entry.hintTypes || entry.hintType),
            detailLine("Occurrences", entry.occurrenceCount),
            detailLine("Files", entry.sourceFiles),
            entry.adminLike ? "Admin/auth-like parameter" : "",
            detailLine("Route key", entry.routeKey)
        ])
    })

    ;(sast.gadgets || []).forEach((entry, index) => {
        const title = `Gadget ${discoveryText(entry.gadgetType, "hint")}`
        const summary = discoveryText(entry.label, "Potential exploit path building block.")
        pushEntry("gadget", entry, index, title, summary, [
            detailLine("Occurrences", entry.occurrenceCount),
            detailLine("Files", entry.sourceFiles),
            detailLine("Route key", entry.routeKey)
        ])
...
function renderSastDiscoveryCard(item, index) {
    const file = discoveryText(item?.file, "")
    const loc = formatRange(item?.location)
    const detailItems = Array.isArray(item?.details) ? item.details.filter(Boolean) : []
    const detailsHtml = detailItems.length
        ? `<ul>${detailItems.map((detail) => `<li>${escapeText(detail)}</li>`).join("")}</ul>`
        : ""
    return `
        <div class="card sast-report-card" data-index="${index}" style="width: 100%;">
            <div class="content">
                <div class="ui blue message" style="margin-bottom: 0px;">
                    <div class="header">
                        <i class="compass icon"></i>
                        ${escapeText(item?.title || `Discovery ${index + 1}`)}
                    </div>
                    <p><b>Category:</b> ${escapeText(item?.category || "discovery")}</p>
                    ${file ? `<p><b>File:</b> ${escapeText(file)}</p>` : ""}
                    ${loc ? `<p><b>Location:</b> ${escapeText(loc)}</p>` : ""}
                </div>
                ${item?.summary ? `<div class="sast-section"><div class="sast-section-title"><strong>Summary</strong></div>${escapeText(item.summary)}</div>` : ""}
                ${detailsHtml ? `<div class="sast-section"><div class="sast-section-title"><strong>Details</strong></div>${detailsHtml}</div>` : ""}
            </div>
        </div>
    `
}
...
function buildRawRequest(request = {}) {
    ...
    const url = request.url || request.ui_url || request.target || "/"
    parts.push(`${method} ${url} HTTP/1.1`)
    const headersBlock = Array.isArray(request.headers) && request.headers.length
        ? request.headers.map(h => `${h.name}: ${h.value}`).join('\n')
        : ''
    if (headersBlock) parts.push(headersBlock)
    const body = typeof request.body === 'string'
        ? request.body
        : (request.body && typeof request.body.preview === 'string' ? request.body.preview : '')
    if (headersBlock || body) parts.push('')
...
function mapDastFindingToLegacy(finding, viewModel) {
    ...
    if (!request.raw) {
        request.raw = buildRawRequest(request)
    }
...
jQuery(function () {
    ...
    const dast_controller = new ptk_controller_dast()
    ...
    function generateSAST(result) {
        ...
        const $discoverySection = $("#sast_report_discovery")
        const $discoveryContainer = $("#sast_report_discovery_items")
        if (!scanResult) {
            $container.html("")
            $discoveryContainer.html("")
            $discoverySection.hide()
            $('#sast_report #sast_discovery_count').text(0)
            replaceSectionFindings("sast", [])
            replaceSectionDiscoveries("sast", [])
        ...
        replaceSectionDiscoveries("sast", discoveryItems)
        $discoveryContainer.html("")
        if (discoveryItems.length) {
            discoveryItems.forEach((item, index) => {
                $discoveryContainer.append(renderSastDiscoveryCard(item, index))
            })
            $discoverySection.show()
        } else {
            $discoverySection.hide()
        }
