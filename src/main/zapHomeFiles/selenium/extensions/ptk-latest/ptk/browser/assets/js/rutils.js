...
async function sendPing(tabId, frameId) {
    const msg = { channel: "ptk_popup2content", type: "ping" }
    const opts = typeof frameId === 'number' ? { frameId } : undefined
    const res = await browser.tabs.sendMessage(tabId, msg, opts)
    return !!res?.ok
}

export async function pingContentScript(tabId, { timeoutMs = 2000, retries = 2 } = {}) {
    if (typeof tabId !== 'number') return false;

    // 1) Try top frame first with retries
    for (let attempt = 0; attempt <= retries; attempt++) {
        const ok = await withTimeout(
            sendPing(tabId).catch(() => false),
            timeoutMs
        )
        if (ok) return true
    }

    // 2) If that fails, try other frames (for iframe-heavy apps like JupyterLab)
    let frames = []
    try {
    ...
    // Filter to valid frames with real URLs, skip about:blank
    const candidateFrames = (frames || [])
        .filter(f => f && typeof f.frameId === 'number')
        .filter(f => f.url && !/^about:blank/i.test(f.url))
        .sort((a, b) => (a.frameId === 0 ? 1 : 0) - (b.frameId === 0 ? 1 : 0))

    for (const f of candidateFrames) {
        for (let attempt = 0; attempt <= 1; attempt++) {
            const ok = await withTimeout(
                sendPing(tabId, f.frameId).catch(() => false),
                timeoutMs
            )
            if (ok) return true
        }
                ...
                <div class="fields" id="finding_source_sink_section" style="min-height: 65%; margin-bottom: 4px; display: none;">
                    <div class="eight wide field" id="finding_source_column" style="padding-right: 1px; overflow:auto">
                        <div class="ui message message-code" style="height:100%;">
                            <div class="meta">
                                <div class="header"><span id="source_header_label">Source</span>: <span id="source_name"></span></div>
                    ...
                    <div class="eight wide field response_view" id="finding_sink_column" style="padding-right: 1px; overflow:auto">
                        <div class="ui message message-code" style="height: 100%;">
                            <div class="header"><span id="sink_header_label">Sink</span>: <span id="sink_name"></span></div>
...
function resetFindingModal() {
    ...
    const sourceHeaderLabel = document.getElementById('source_header_label');
    const sinkHeaderLabel = document.getElementById('sink_header_label');
    if (sourceHeaderLabel) sourceHeaderLabel.textContent = 'Source';
...
function setFindingMetadata(description, recommendation, links, extras = {}) {
    $("#attack_description").html(sanitizeHtml(description || "", RICH_TEXT_SANITIZE_CONFIG));
    $("#attack_recommendation").html(sanitizeHtml(recommendation || "", RICH_TEXT_SANITIZE_CONFIG));
    ...
    const aggregateSummary = extras?.aggregateSummary && typeof extras.aggregateSummary === "object"
        ? extras.aggregateSummary
        : null
    if (aggregateSummary) {
        const occurrenceCount = Number.isFinite(Number(aggregateSummary.occurrenceCount))
            ? Number(aggregateSummary.occurrenceCount)
            : null
        const sampleCount = Number.isFinite(Number(aggregateSummary.sampleCount))
            ? Number(aggregateSummary.sampleCount)
            : null
        const sampleLimit = Number.isFinite(Number(aggregateSummary.sampleLimit))
            ? Number(aggregateSummary.sampleLimit)
            : null
        const truncated = aggregateSummary.truncated === true
        const bits = []
        if (occurrenceCount !== null) {
            bits.push(`${occurrenceCount} affected response${occurrenceCount === 1 ? '' : 's'}`)
        }
        if (sampleCount !== null) {
            let sampleLabel = `${sampleCount} sampled URL${sampleCount === 1 ? '' : 's'}`
            if (sampleLimit !== null) {
                sampleLabel += ` (limit ${sampleLimit})`
            }
            if (truncated) {
                sampleLabel += ', truncated'
            }
            bits.push(sampleLabel)
        }
        if (bits.length) {
            extrasMarkup.push(`<div class="item"><strong>Aggregation:</strong> ${ptk_utils.escapeHtml(bits.join(' | '))}</div>`)
        }
    }
    extrasMarkup.forEach(html => linksContainer.insertAdjacentHTML("beforeend", sanitize(html)))
    const affectedUrls = Array.isArray(extras?.affectedUrls)
        ? extras.affectedUrls.filter((entry) => entry && typeof entry === "object" && entry.url)
        : []
    if (affectedUrls.length) {
        const item = document.createElement("div")
        item.className = "item"
        const title = document.createElement("strong")
        title.textContent = "Affected routes: "
        item.appendChild(title)
        const list = document.createElement("div")
        list.className = "ui relaxed list"
        affectedUrls.forEach((entry) => {
            const row = document.createElement("div")
            row.className = "item"
            const method = String(entry.method || "").trim().toUpperCase()
            let routeLabel = String(entry.url || "")
            try {
                const parsed = new URL(String(entry.url), document?.baseURI || "http://localhost")
                routeLabel = `${method || "GET"} ${parsed.pathname || "/"}${parsed.search || ""}${parsed.hash || ""}`
            } catch (_) {
                routeLabel = `${method ? `${method} ` : ""}${String(entry.url)}`
            }
            row.appendChild(makeSafeAnchor(String(entry.url), routeLabel))
    ...
    if (!entries.length) {
        if (!extrasMarkup.length && !affectedUrls.length) {
...
function isDastReportableAttack(info) {
    const meta = info?.metadata && typeof info.metadata === "object" ? info.metadata : {}
    if (meta.reportFinding === false) return false
    const ruleId = String(info?.ruleId || meta.id || meta.ruleId || info?.id || "").toLowerCase()
...
function sanitizeHtml(value, cfg) {
    if (dompurify && typeof dompurify.sanitize === 'function') {
        return dompurify.sanitize(value, cfg);
    }
    return String(value ?? '');
}
...
function renderSnippetWithHighlight(snippetRaw, needle, highlightClass, loc = null, options = {}) {
    ...
    if (targetIdx === -1) {
        ...
        return sanitizeHtml(html, BASE_SANITIZE);
    }
...
const makeLinkItem = (title, url) => {
    ...
    const hrefText = String(url || '').trim();
    if (titleText && titleText.toLowerCase() !== hrefText.toLowerCase()) {
...
export function resolveConfidence(info) {
    const candidates = [
        info?.confidence,
...
function renderConfidenceBadge(confidence) {
    const value = formatConfidenceValue(confidence);
    if (value === null) return "";
    return `<span class="ui tiny label ptk-confidence-badge">Confidence: ${value}</span>`;
}
...
export function renderFindingHeader(titleHtml, confidence) {
    const badge = renderConfidenceBadge(confidence);
    return `
        <div class="ptk-finding-header">
            <div class="ptk-finding-header-main">${titleHtml || ""}</div>
            ${badge ? `<div class="ptk-finding-header-side">${badge}</div>` : ""}
        </div>
    `;
}

function buildAttackAggregateKey(info, original) {
    const moduleId = String(info?.moduleId || info?.finding?.moduleId || info?.metadata?.moduleId || '').trim().toLowerCase()
    if (moduleId !== 'headers') return ''
    const aggregateMode = String(
        info?.presentationAggregate
        || info?.finding?.presentationAggregate
        || info?.metadata?.presentationAggregate
        || info?.metadata?.extensions?.ptk?.presentation?.aggregate
        || info?.finding?.metadata?.extensions?.ptk?.presentation?.aggregate
        || info?.finding?.extensions?.ptk?.presentation?.aggregate
        || ''
    ).trim().toLowerCase()
    if (aggregateMode && aggregateMode !== 'scan') return ''
    const ruleId = String(info?.ruleId || info?.finding?.ruleId || info?.metadata?.ruleId || info?.metadata?.id || '').trim().toLowerCase()
    const aggregateScopeHost = String(info?.finding?.evidence?.dast?.aggregate?.scopeHost || '').trim().toLowerCase()
    if (moduleId && ruleId && aggregateScopeHost) {
        return ['scan', moduleId, ruleId, aggregateScopeHost].join('|')
    }
    const requestUrl = String(info?.request?.url || original?.request?.url || info?.finding?.location?.runtimeUrl || info?.finding?.location?.url || '').trim()
    if (!moduleId || !ruleId || !requestUrl) return ''
    try {
        const parsed = new URL(requestUrl, document?.baseURI || 'http://localhost')
        const host = String(parsed.host || '').trim().toLowerCase()
        return host ? ['scan', moduleId, ruleId, host].join('|') : ''
    } catch (_) {
        return ''
    }
}

function getAttackPresentationAggregateMode(info) {
    return String(
        info?.presentationAggregate
        || info?.finding?.presentationAggregate
        || info?.metadata?.presentationAggregate
...
function buildStandardFindingAggregateMeta(info, original) {
    if (getAttackPresentationAggregateMode(info) !== 'scan') return null
    const finding = info?.finding && typeof info.finding === 'object' ? info.finding : null
    const dastEvidence = finding?.evidence?.dast && typeof finding.evidence.dast === 'object'
        ? finding.evidence.dast
        : {}
    const samples = Array.isArray(dastEvidence.samples)
        ? dastEvidence.samples.filter((entry) => entry && typeof entry === 'object' && entry.url)
        : []
    ...
    const occurrenceCount = Number.isFinite(Number(dastEvidence.occurrenceCount))
        ? Number(dastEvidence.occurrenceCount)
        : null
    const sampleCount = samples.length
    const sampleLimit = Number.isFinite(Number(dastEvidence.sampleLimit))
        ? Number(dastEvidence.sampleLimit)
        : null
    const truncated = dastEvidence.truncated === true
    const routes = []
    const routeKeys = new Set()
    for (const sample of samples) {
        const rawUrl = String(sample?.url || '').trim()
        if (!rawUrl) continue
        let routePath = rawUrl
        try {
            const parsed = new URL(rawUrl, document?.baseURI || 'http://localhost')
...
function renderAggregateRouteListHtml(routes = [], {
    ...
    const visibleRoutes = routes.slice(0, Math.max(1, maxVisible))
    const hiddenRoutes = routes.slice(Math.max(1, maxVisible))
    const routeItems = visibleRoutes.map((route) => {
        const label = `${String(route?.method || '').trim().toUpperCase() || 'GET'} ${String(route?.path || '/').trim() || '/'}`
        const href = String(route?.url || '').trim()
        if (href) {
            return `<li><a class="ptk-card-link" href="${ptk_utils.escapeHtml(href)}" target="_blank" rel="noreferrer">${ptk_utils.escapeHtml(label)}</a></li>`
        }
        return `<li>${ptk_utils.escapeHtml(label)}</li>`
    }).join('')
    const hiddenRouteItems = hiddenRoutes.map((route) => {
        const label = `${String(route?.method || '').trim().toUpperCase() || 'GET'} ${String(route?.path || '/').trim() || '/'}`
        const href = String(route?.url || '').trim()
        if (href) {
            return `<li><a class="ptk-card-link" href="${ptk_utils.escapeHtml(href)}" target="_blank" rel="noreferrer">${ptk_utils.escapeHtml(label)}</a></li>`
    ...
    const matchedResponses = Number.isFinite(Number(occurrenceCount)) ? Number(occurrenceCount) : null
    const summaryText = matchedResponses !== null && matchedResponses > routeCount
        ? `Affected routes: <b>${ptk_utils.escapeHtml(String(routeCount))}</b> | Matched responses: <b>${ptk_utils.escapeHtml(String(matchedResponses))}</b>`
        : `Affected routes: <b>${ptk_utils.escapeHtml(String(routeCount))}</b>`
    const safeToggleKey = ptk_utils.escapeHtml(String(toggleKey || ''))
    return `
        <div class="description">
            <p>${summaryText}</p>
            <ul style="margin: 6px 0 0 16px; overflow-wrap:anywhere; word-break:break-word;">${routeItems}</ul>
            ${hiddenRoutes.length ? `
                <div class="grouped_recon_routes_all" data-grouped-routes-key="${safeToggleKey}" style="display:none;">
                    <ul style="margin: 6px 0 0 16px; overflow-wrap:anywhere; word-break:break-word;">${hiddenRouteItems}</ul>
                </div>
                <p style="margin-top:6px;">
                    <a href="#" class="toggle_grouped_recon_routes" data-grouped-routes-key="${safeToggleKey}" data-expanded="0">More</a>
                </p>
            ` : ''}
        </div>
    `
...
function renderAttackProof(proof) {
    const raw = proof === undefined || proof === null ? '' : String(proof)
    if (!raw) return ''
    return `<div class="description"><p>Proof:</p><div class="ptk-proof-snippet" style="display:block; max-height:120px; overflow:auto; white-space:pre-wrap; overflow-wrap:anywhere; word-break:break-word;"><code>${ptk_utils.escapeHtml(raw)}</code></div></div>`
}

function renderAttackConfirmation(info) {
    const confirmation =
        (info?.confirmation && typeof info.confirmation === 'object' ? info.confirmation : null)
        || (info?.metadata?.confirmation && typeof info.metadata.confirmation === 'object' ? info.metadata.confirmation : null)
        || (info?.finding?.evidence?.dast?.confirmation && typeof info.finding.evidence.dast.confirmation === 'object' ? info.finding.evidence.dast.confirmation : null)
        || (info?.finding?.evidence?.dast?.meta?.confirmation && typeof info.finding.evidence.dast.meta.confirmation === 'object' ? info.finding.evidence.dast.meta.confirmation : null)
        || null
    if (!confirmation) return ''
    const tierRaw = String(confirmation.tier || '').trim().toLowerCase()
    const tierLabel = tierRaw === 'executed'
        ? 'Executed'
        : tierRaw === 'retrieved' || tierRaw === 'retrievable'
            ? 'Retrievable'
            : tierRaw === 'uploaded' || tierRaw === 'accepted'
                ? 'Accepted'
                : ''
    if (!tierLabel) return ''
    const url = String(confirmation.url || '').trim()
    const viaHtml = url
        ? ` via <a class="ptk-card-link" href="${ptk_utils.escapeHtml(url)}" target="_blank" rel="noreferrer">${ptk_utils.escapeHtml(url)}</a>`
        : ''
    return `<div class="description"><p>Confirmation: <b>${ptk_utils.escapeHtml(tierLabel)}</b>${viaHtml}</p></div>`
}
...
export function bindAttack(info, original, index, requestId = -1) {
    ...
    if (info.proof) {
        proof = renderAttackProof(info.proof);
    }
    confirmation = renderAttackConfirmation(info)
    ...
    const aggregateMeta = buildStandardFindingAggregateMeta(info, original)
    const targetUrl = String(
        aggregateMeta?.sampleUrl
        || info.request?.url
        || original?.request?.url
        || ''
    ).trim()
    let target = targetUrl ? ptk_utils.escapeHtml(targetUrl) : "";
    const targetLabel = aggregateMeta ? 'Sample URL' : 'URL'
    const safeRequestId = requestId === null || requestId === undefined || requestId === ''
        ? '__ptk_unknown__'
        : String(requestId)
    const findingIdAttr = String(info?.findingId || info?.finding?.id || '').trim()
    const aggregateKeyAttr = buildAttackAggregateKey(info, original)
    const confidence = resolveConfidence(info)
    const titleHtml = aggregateMeta
        ? `${icon} <span class="ui mini basic label">${ptk_utils.escapeHtml(aggregateMeta.groupedLabel)}</span>`
        : icon
    const aggregateSummaryHtml = aggregateMeta
        ? renderAggregateRouteListHtml(aggregateMeta.routes, {
            occurrenceCount: aggregateMeta.occurrenceCount,
            truncated: aggregateMeta.truncated,
            toggleKey: aggregateKeyAttr
        })
        : ''
    let item = `
                <div class="ui message attack_info ${attackClass} ${ptk_utils.escapeHtml(safeRequestId)}"  data-order="${order}" data-severity="${ptk_utils.escapeHtml(severityAttr)}" data-request-id="${ptk_utils.escapeHtml(safeRequestId)}" data-finding-id="${ptk_utils.escapeHtml(findingIdAttr)}" data-aggregate-key="${ptk_utils.escapeHtml(aggregateKeyAttr)}">
                ${renderFindingHeader(titleHtml, confidence)}
                <div class="description">
                    <p>${ptk_utils.escapeHtml(targetLabel)}: <a class="ptk-card-link" href="${target}" target="_blank" rel="noreferrer">${target}</a></p>
                </div>
                ${aggregateSummaryHtml}
                ${proof}
                ${confirmation}
                ${param}
...
export function bindIASTAttack(info, requestId = -1) {
    ...
    return `
        <div class="ui message attack_info ${attackClass} iast_attack_card ${requestId}" style="overflow:auto" data-order="${order}" data-severity="${attr(severity)}"${requestAttr}>
            ${renderFindingHeader(icon, confidence)}
            <div class="description">
...
export function bindAttackDetails_IAST(info = {}) {
    ...
    setFindingModalTitle(sanitizeHtml(modalTitle, MODAL_TITLE_SANITIZE_CONFIG))
...
export function bindSASTAttack(info, index = -1) {
    ...
    const findingKind = String(info?.findingKind || info?.metadata?.findingKind || '').toLowerCase();
    const attr = (val) => ptk_utils.escapeHtml(String(val || ""));
    const isHintLike = findingKind === 'hint' || findingKind === 'artifact';
    const sourceName = String(info?.source?.sourceName || info?.source?.label || '').trim();
    const sinkName = String(info?.sink?.sinkName || info?.sink?.label || '').trim();
    const primaryFile = info?.sink?.sinkFile || info?.source?.sourceFile || info?.pageUrl || '';
    const primaryLoc = info?.sink?.sinkLoc || info?.source?.sourceLoc || null;
    const pageUrl = String(info?.pageUrl || info?.pageCanon || '').trim();
    const primaryFileRaw = String(primaryFile || '').trim();
    const primaryFileCanon = canonicalizeSastFileId(primaryFileRaw);
    const pageDisplay = pageUrl && (primaryFileRaw.startsWith('inline') || !primaryFileCanon || pageCanon !== primaryFileCanon)
        ? `<div>Page: <b><i>${sanitize(pageUrl)}</i></b></div>`
        : '';
    const locationText = Number.isFinite(primaryLoc?.line)
        ? `line ${primaryLoc.line}${Number.isFinite(primaryLoc?.column) ? `:${primaryLoc.column}` : ''}`
        : '';
    const sameExpression = !!sourceName && !!sinkName && sourceName === sinkName;
    const detailLines = isHintLike
        ? [
            sameExpression
                ? `<div>Matched: <b><i>${sanitize(sourceName)}</i></b></div>`
                : (sourceName ? `<div>Observed: <b><i>${sanitize(sourceName)}</i></b></div>` : ''),
            (!sameExpression && sinkName) ? `<div>Pattern: <b><i>${sanitize(sinkName)}</i></b></div>` : '',
            pageDisplay,
            primaryFile ? `<div>File: <b><i>${sanitize(primaryFile)}</i></b></div>` : '',
            locationText ? `<div>Location: <b><i>${sanitize(locationText)}</i></b></div>` : '',
        ].filter(Boolean).join('')
        : `
...
export function bindAttackDetails_SAST(el, info) {
    ...
    if (sourceHeaderLabel) sourceHeaderLabel.textContent = isHintLike ? 'Observed' : 'Source';
    if (sinkHeaderLabel) sinkHeaderLabel.textContent = isHintLike ? 'Matched' : 'Sink';
...
export function bindAttackDetails_DAST(el, attack, original) {
    ...
    const dastEvidence = finding?.evidence?.dast && typeof finding.evidence.dast === "object"
        ? finding.evidence.dast
        : {}
    const aggregateSamples = Array.isArray(dastEvidence?.samples)
    ...
    setFindingMetadata(description, recommendation, links, {
        ...
        aggregateSummary: finding?.presentationAggregate === "scan"
            ? {
                occurrenceCount: dastEvidence?.occurrenceCount,
                sampleCount: aggregateSamples.length,
                sampleLimit: dastEvidence?.sampleLimit,
                truncated: dastEvidence?.truncated === true
            }
            : null,
        affectedUrls: aggregateSamples
    });
...
export function showEvidenceFindingDetails(details = {}) {
    ...
    if (engine === 'DAST' && (details?.attack || details?.original || finding)) {
        const attack = details?.attack
            ? Object.assign({}, details.attack, {
                finding: details.attack.finding || finding || null
            })
            : { finding };
